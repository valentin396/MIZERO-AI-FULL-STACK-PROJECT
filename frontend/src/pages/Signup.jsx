...
export default function Signup() {
  ...
  return (
    <div className="max-w-sm mx-auto px-6 py-16">
      ...
      <p className="text-ink/60 text-sm mb-6">Needed so your reports are yours — you can edit or resolve them later.</p>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input required value={form.phone} onChange={update('phone')} placeholder="07XXXXXXXX"
            className="w-full border border-ink/20 px-3 py-2 bg-cream focus:outline-none focus:border-clay" />
        </div>
        <div>
        ...
        <button type="submit" disabled={loading}
          className="w-full bg-clay text-cream py-2.5 rounded-lg font-medium hover:bg-clay-dark transition-colors disabled:opacity-60">
          {loading ? 'Creating account…' : 'Sign up'}
        </button>
      </form>
      <p className="text-sm text-ink/60 mt-6">
        Already have an account? <Link to="/login" className="text-clay hover:underline">Log in</Link>
      </p>
    </div>
  );
}
